...
class AMX_MOE_BASE {
 public:
  ...
  template <typename... Args>
  bool verify_install_against_loaded(Args&&... args) {
    return derived()->verify_install_against_loaded(std::forward<Args>(args)...);
  }
      ...
      for (int i = 0; i < m_local_num_[expert_idx]; i++) {
        ...
        for (int j = n_start; j < n_end; j += 32) {
          ...
          __m512 result0 = amx::act_fn(gate_val0, up_val0, config_.swiglu_limit, config_.swiglu_alpha,
                                       config_.situ_beta, config_.situ_linear_beta);

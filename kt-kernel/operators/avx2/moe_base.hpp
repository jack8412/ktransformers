    ...
    auto fn = [this, nth](int task_id) {
      ...
      for (int i = 0; i < m_local_num_[expert_idx]; i++) {
        ...
        for (; j + 8 <= n_end; j += 8) {
          ...
          __m256 result = avx2::act_fn(gate_val, up_val, swiglu_limit, swiglu_alpha, situ_beta, situ_linear_beta);
          avx2::store_fp32_to_bf16(gate_ptr + j, result);
        }
        // Scalar tail — mirror the vectorized situ / swigluoai / silu paths in avx2::act_fn.
        for (; j < n_end; j++) {
          float g = GGML_BF16_TO_FP32(gate_ptr[j]);
          float u = GGML_BF16_TO_FP32(up_ptr[j]);
          if (situ_beta > 0.0f) {
            float situ_a = situ_beta * tanhf(g / situ_beta) / (1.0f + expf(-g));
            if (situ_linear_beta > 0.0f) {
              u = situ_linear_beta * tanhf(u / situ_linear_beta);
            }
            gate_ptr[j] = GGML_FP32_TO_BF16(situ_a * u);
